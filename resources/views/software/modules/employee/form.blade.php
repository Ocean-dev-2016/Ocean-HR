                    ...
                    <div class="col-md-3 col-sm-12 mb-2">
                        <div class="form-group">
                            ...
                            <select name="role_id" id="role_id" placeholder="Team Role"
                                ...
                                @if (isset($team_roles) && count($team_roles) > 0)
                                    @foreach ($team_roles as $role)
                                        <option value="{{ $role->id }}"
                                            {{ (isset($edit) && $edit->role_id == $role->id) || old('role_id') == $role->id ? 'selected' : '' }}>
                                            {{ $role->name }}
                                        </option>
                                    @endforeach
                                @endif
...
@push('page_scripts')
    ...
    <script>
                ...
                success: function(response) {
                    if (response.status && response.data) {
                        ...
                        if (isManual) {
                            ...
                            // employeeCodeInput.val(''); // Clear auto-generated value
                        } else {
        ...
        function getEmployeeCode() {
            ...
            let parent_type_id = $(this).attr('data-parent_type_id') || 'all';
            ...
            // Check company setting first - only generate if auto mode
            $.ajax({
                type: 'GET',
                url: '{{ env('API_URL') }}get-company-setting',
                data: {
                ...
                success: function(settingResponse) {
                    if (settingResponse.status && settingResponse.data) {
                        let isManual = settingResponse.data.employee_code_auto_generation === 'manual';
                        console.log("L-697", isManual);

                        if (isManual) {
                            // Manual mode - don't generate code
                            // $('#employee_code').val('');
                            // return;
                        }
                    }

                    // Auto mode - generate code
                    formJson = {
                        ...formJson,
                        company_id: company_id
                    };
                    ...
                    $.ajax({
                        ...
                        data: formJson,
                        success: function(response) {
                            console.log("Employee code response:", response);
                            if (response.status && response.data) {
                                setTimeout(function() {
    ...
    <script>
        $(document).ready(function() {
            function toggleAnniversary() {
                let status = $("#marital_status").val();
                if (status === "married") {
                    $("#anniversary_div").show();
                } else {
                    $("#anniversary_div").hide();
                    $("#date_of_anniversary").val(""); // clear if hidden
                }
            }
            ...
            // Run on change
            $("#marital_status").on("change", toggleAnniversary);
        });
