...
@php
    ...
    if (count(config('constants.app_permissions'))) {
        $permissions = config('constants.app_permissions');
    }
@endphp
...
@section('content')
    ...
    <div class="card my-3 mb-4">
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-3">
                    <b>Company Name : </b> {{ $team_person?->company?->company_name ?? '-' }}
                </div>
                <div class="col-md-3">
                    <b>Team Person Name : </b> {{ $team_person?->name ?? '-' }}
                </div>
                <div class="col-md-3">
                    <b>Parent : </b> {{ $team_person?->team_role?->parent_name ?? '-' }}
                </div>
                <div class="col-md-3">
                    <b>Team Role : </b> {{ $team_person?->team_role?->name ?? '-' }}
                </div>
            </div>
            <hr>
            <form action="{{ route($route . '.store-permission') }}" method="POST" id="forminfo">
                ...
                <input type="hidden" id="team_role_id" name="team_role_id" value="{{ $team_person?->team_role?->id ?? '' }}">
                <input type="hidden" id="team_id" name="team_id" value="{{ $team_person?->id ?? '' }}">
                <input type="hidden" id="company_id" name="company_id" value="{{ $team_person->company_id ?? '' }}">
                                    ...
                                    @foreach ($panel_sub_modules as $item)
                                        @if ($item && $item?->sub_menus && count($item?->sub_menus))
                                            ...
                                            @foreach ($item?->sub_menus as $item_sub_menu)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td data-company_id="{{ $team_person?->company_id ?? '-' }}"
                                                        data-team_role_id="{{ $team_person?->team_role?->id ?? '' }}"
                                                        data-team_id="{{ $team_person?->id ?? '' }}"
                                                    ...
                                                    @foreach ($permissions as $permission_key => $permission_value)
                                                        @php
                                                            ...
                                                            $tr_uuid = [];
                                                            $tr_uuid[] = $team_person?->company_id;
                                                            $tr_uuid[] = $team_person?->team_role?->id;
                                                            $tr_uuid[] = $team_person?->id;
