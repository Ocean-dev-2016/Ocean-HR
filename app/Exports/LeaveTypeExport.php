...
class LeaveTypeExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    protected $srNo = 1;
    ...
    public function collection()
    {
        ...
        if (isset($this->filter_params->search) && $this->filter_params->search !== '') {
            $query->where(function ($query1) {
                $query1->where((new LeaveType())->getTable() . '.sort_name', 'like', "%" . $this->filter_params->search . "%")->orwhere((new LeaveType())->getTable() . '.full_name', 'like', "%" . $this->filter_params->search . "%");
            });
        }
    ...
    public function map($row): array
    {
        ...
        $rowData = [
            ...
            $row->full_name,
            $row->count,
            $row->mode == 1 ? "Comapny Pay" : "Employee Pay",
            $row->carry_forward == 1 ? "Yes" : "No",
            $row->status
        ];
