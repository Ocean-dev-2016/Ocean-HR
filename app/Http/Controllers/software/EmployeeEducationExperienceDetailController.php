...
use App\Exports\EmployeeEducationExperienceDetailExport;
use App\Helpers\Helper;
use App\Http\Controllers\Controller;
use App\Http\Requests\EmployeeEducationExperienceRequest;
...
class EmployeeEducationExperienceDetailController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            'title' => 'Employee Education & Experience Detail',
            'folder_path' => 'software.modules.employee.employee-education-experience',
            'route' => 'employee-education-experience',
            'table_name' => (new EmployeeEducationExperienceDetail())->getTable(),
            'permisstion_prefix' => 'employee-education-experience',
            'module_name' => 'Education Experience',
            ...
            if ($request->ajax()) {
                ...
                $returnData = Datatables::of($data)
                    ...
                    ->filter(function ($query) use ($request) {
                        ...
                        if ($request->has('filter_department_name') && $request->filter_department_name) {
                            $query->where('department_name', $request->filter_department_name);
                        }

                        if ($request->has('filter_designation_name') && $request->filter_designation_name) {
                            $query->where('designation_name', $request->filter_designation_name);
                        }

                        if ($request->has('filter_document_type') && $request->filter_document_type) {
                            $query->where('document_type', $request->filter_document_type);
                        }

                        $query->when($request->filled('search'), function ($q) use ($request) {
                            $q->where('degree', 'like', '%' . $request->search . '%')->orWhere('company_name', 'like', '%' . $request->search . '%')->orWhere('unit_change', 'like', '%' . $request->search . '%')->orWhere('document_name', 'like', '%' . $request->search . '%');
                        });
                        if ($request->has('status') && $request->status !== null && $request->status !== 'all') {
                            $query->where((new EmployeeEducationExperienceDetail())->getTable() . '.status', $request->status);
                        }
                    })
     ...
     */
    public function store(EmployeeEducationExperienceRequest $request)
    {
        $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
        ...
        try {
            ...
            if ($request->hasFile('attachment')) {
                ...
                $year = now()->format('Y');
                $month = now()->format('m');
                $folder = "uploads/" . $validated['company_id'] . "-" . $company_slug . "/EmployeeEducationExperience/{$year}-{$month}" . "/";
            ...
            EmployeeEducationExperienceDetail::create($validated);
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            if (!empty($modules['company_id'])) {
                $eeedQuery->where('company_id', $modules['company_id']);
            }
            $edit = $eeedQuery->findOrFail($id);
     ...
     */
    public function update(EmployeeEducationExperienceRequest $request, string $id)
    {
        $modules = $this->modules;
        ...
        try {
            ...
            $updateData = $eeedQuery->findOrFail($id);
    ...
    public function destroy(string $id, Request $request)
    {
        ...
        $dataDelete = $eeedQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            $eeedQuery = EmployeeEducationExperienceDetail::withTrashed();
            if (!empty($modules['company_id'])) {
                $eeedQuery->where('company_id', $modules['company_id']);
            }
            $restore_data = $eeedQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $country = $eeedQuery->findOrFail($request?->id);
            if ($country) {
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            $query->when($request->has('filter_department_name') && $request->filter_department_name, function ($q) use ($request) {
                $q->where('department_name', $request->filter_department_name);
            });

            $query->when($request->has('filter_designation_name') && $request->filter_designation_name, function ($q) use ($request) {
                $q->where('designation_name', $request->filter_designation_name);
            });

            $query->when($request->has('filter_document_type') && $request->filter_document_type, function ($q) use ($request) {
                $q->where('document_type', $request->filter_document_type);
            });

            // 🔍 Text Search
            $query->when($request->filled('search'), function ($q) use ($request) {
                $q->where(function ($inner) use ($request) {
                    $inner->where('degree', 'like', '%' . $request->search . '%')
                        ->orWhere('company_name', 'like', '%' . $request->search . '%')
                        ->orWhere('unit_change', 'like', '%' . $request->search . '%')
            ...
            return view($modules['folder_path'] . '.print', compact('employeeDocuments', 'company_id', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new EmployeeEducationExperienceDetailExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Employee Education & Experience Details-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
}
