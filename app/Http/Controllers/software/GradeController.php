...
class GradeController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            'title' => 'Grade Master',
            'folder_path' => 'software.modules.master.grade',
            'route' => 'grades',
            'table_name' => (new Grade())->getTable(),
            'permisstion_prefix' => 'Grade Master',
                ...
                $data = Grade::select('*')
                    ->where(function ($query) use ($modules, $loginUserId) {
                        if (Auth::guard('employees')->check() || !empty($modules['company_id'])) {
                            ...
                            $query->where((new Grade())->getTable() . '.company_id', $companyId);

                            if ($modules['personal_data_permission'] && $modules['all_data_permission'] == false) {
                                $query->where((new Grade())->getTable() . '.created_by', $loginUserId);
                            }
                        }
                    })
                    ->orderBy('id', 'DESC');
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            if (!empty($modules['company_id'])) {
                $gradeQuery->where('company_id', $modules['company_id']);
            }
            $edit = $gradeQuery->findOrFail($id);
    ...
    public function update(GradeRequest $request, string $id)
    {
        ...
        try {
            ...
            $updateData = $gradeQuery->findOrFail($id);
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        $dataDelete = $gradeQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        $modules = $this->modules;
        try {
            $gradeQuery = Grade::withTrashed();
            if (!empty($modules['company_id'])) {
                $gradeQuery->where('company_id', $modules['company_id']);
            }
            $restore_data = $gradeQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $grade = $gradeQuery->findOrFail($request->id);
            $grade->status = $request->update_status;
            $grade->save();
            
            if ($request->ajax()) {
                return $this->sendResponse($grade, $modules['title'] . ' status updated successfully.');
            }
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            $grades = $query->get();
            return view($modules['folder_path'] . '.print', compact('grades', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
    ...
    {
        $modules = $this->modules;
        return Excel::download(new GradeExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Grade-' . date("Ymd-His") . '.xlsx');
    }
}
