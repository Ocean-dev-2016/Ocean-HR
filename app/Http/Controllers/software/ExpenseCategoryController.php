...
class ExpenseCategoryController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            ...
            'folder_path' => 'software.modules.expense.expense-category',
            'route' => 'expense-category',
            'table_name' => (new ExpenseCategory())->getTable(),
            'permisstion_prefix' => 'expense-category',
    ...
    public function index(Request $request)
    {
        ...
        try {
            $columns = [
                ...
                (object) ['data' => 'DT_RowIndex', 'name' => 'DT_RowIndex', 'td_label' => 'Sr No.', 'orderable' => false, 'searchable' => false, 'className' => 'w-10'],

                (object) ['data' => "company.company_name", 'name' => 'company_id', 'td_label' => 'Company Name', 'className' => ''],
                (object) ['data' => "branch.name", 'name' => 'branch_id', 'td_label' => 'Branch Name', 'className' => ''],
                (object) ['data' => "name", 'name' => 'name', 'td_label' => 'Expense Category Name', 'className' => ''],
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            $ecQuery = ExpenseCategory::query();
            if (!empty($modules['company_id'])) {
                $ecQuery->where('company_id', $modules['company_id']);
            }
            $edit = $ecQuery->findOrFail($id);
    ...
    public function update(ExpenseCategoryRequest $request, string $id)
    {
        ...
        try {
            ...
            $updateData = $ecQuery->findOrFail($id);
            if ($updateData) {
                unset($validated['id']);
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        $dataDelete = $ecQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            $ecQuery = ExpenseCategory::withTrashed();
            if (!empty($modules['company_id'])) {
                $ecQuery->where('company_id', $modules['company_id']);
            }
            $country = $ecQuery->findOrFail($id);
            $country->restore();
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $country = $ecQuery->findOrFail($request?->id);
            if ($country) {
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new ExpenseCategoryExport($request->all(), $this->authenticateLoginUserDetails, $modules, $loginUserId), 'ExpenseCategory-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
        ...
        try {
            ...
            $query = ExpenseCategory::with('company')
                ...
                ->where(function ($query) use ($modules, $loginUserId) {
                    if (Auth::guard('employees')->check() && $modules['company_id']) {
            ...
            return view($modules['folder_path'] . '.print', compact('ExpenseCategory', 'company_id', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
