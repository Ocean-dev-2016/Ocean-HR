...
class IncentiveController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            'title' => 'Incentive',
            'folder_path' => 'software.modules.master.incentive',
            'route' => 'incentive',
            'table_name' => (new Incentive())->getTable(),
            'permisstion_prefix' => 'Incentive',
    ...
    public function index(Request $request)
    {
        ...
        try {
            $columns = [
                ...
                (object)['data' => "incentive_value", 'name' => 'incentive_value', 'td_label' => 'Incentive Value', 'className' =>  ''],
                (object)['data' => "incentive_amount", 'name' => 'incentive_amount', 'td_label' => 'Incentive Amount', 'className' =>  ''],
            ...
            if ($request->ajax()) {
                ...
                $returnData = Datatables::of($data)
                    ...
                    ->filter(function ($query) use ($request) {
                        ...
                        if ($request->has('search') && $request->search != '') {
                            ...
                            $query->where(function ($q) use ($search) {
                                $q->where('incentive_value', 'like', "%{$search}%")
                    ...
                    ->addColumn('action', function ($row) use ($modules) {
                        ...
                        // $btn .= '<a href="javascript:void(0)" class="btn btn-icon btn-info mr-2"><i class="fa-solid fa-key"></i></a>';
                        if ($btn == '') {
                            $btn = '-';
                        }
                        return $btn;
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            $insQuery = Incentive::query();
            if (!empty($modules['company_id'])) {
                $insQuery->where('company_id', $modules['company_id']);
            }
            $edit = $insQuery->findOrFail($id);
     ...
     */
    public function update(IncentiveRequest $request, string $id)
    {
        $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
        ...
        try {
            ...
            $updateData = $insQuery->findOrFail($id);
            if ($updateData) {
                unset($validated['id']);
    ...
    public function destroy(string $id, Request $request)
    {
        ...
        $dataDelete = $insQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            $insQuery = Incentive::withTrashed();
            if (!empty($modules['company_id'])) {
                $insQuery->where('company_id', $modules['company_id']);
            }
            $restore_data = $insQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $country = $insQuery->findOrFail($request?->id);
            if ($country) {
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            return view($modules['folder_path'] . '.print', compact('incentives', 'company_id', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new IncentiveExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Incentive-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
}
