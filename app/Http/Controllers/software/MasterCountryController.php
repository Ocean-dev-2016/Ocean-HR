...
use App\Http\Requests\MasterCountryRequest;
...
use App\Exports\MasterCountryExport;
...
class MasterCountryController extends Controller
{
    ...
    public function __construct()
    {
        $this->modules = [
            'title' => 'Country',
            'folder_path' => 'software.modules.master.country',
              ...
              if ($request->ajax()) {
                ...
                $returnData = Datatables::of($data)
                    ...
                    ->filter(function ($query) use ($request) {
                        ...
                        // Status Filter (skip if value is 'all')
    ...
    public function store(MasterCountryRequest $request)
    {
        $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
    ...
    public function update(MasterCountryRequest $request, string $id)
    {
         $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new MasterCountryExport($request->all()), 'master_countries-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            $countries = $query->orderBy('id', 'DESC')->get();

            return view($modules['folder_path'] . '.print', compact('countries'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
