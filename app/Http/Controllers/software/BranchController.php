...
class BranchController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            'title' => 'Branch',
            'folder_path' => 'software.modules.master.branch',
            'route' => 'branches',
            'table_name' => (new Branch())->getTable(),
            'permisstion_prefix' => 'branches',
    ...
    public function index(Request $request)
    {
        ...
        try {
            // Columns for the view / JS (IMPORTANT: names must match DB columns for ordering)
            $columns = [
                ...
                (object)['data' => 'name',  'name' => (new Branch())->getTable() . '.name', 'td_label' => 'Name', 'className' => '', 'orderable' => true, 'searchable' => false],
                (object)['data' => 'prefix', 'name' => (new Branch())->getTable() . '.prefix', 'td_label' => 'Prefix', 'className' => '', 'orderable' => true, 'searchable' => false],
                (object)['data' => 'employee_code_start', 'name' => (new Branch())->getTable() . '.employee_code_start', 'td_label' => 'Employee Code Start', 'className' => '', 'orderable' => true, 'searchable' => false],
                (object)['data' => (new Branch())->getTable() . '.canteen_max_time',  'name' => (new Branch())->getTable() . '.canteen_max_time', 'td_label' => 'Canteen Max Hours', 'className' => 'text-center', 'orderable' => true, 'searchable' => false],
                (object)['data' => (new Branch())->getTable() . '.canteen_min_time',  'name' => (new Branch())->getTable() . '.canteen_min_time', 'td_label' => 'Canteen Min Hours', 'className' => 'text-center', 'orderable' => true, 'searchable' => false],
                (object)['data' => 'status', 'name' => (new Branch())->getTable() . '.status', 'td_label' => 'Status', 'className' => 'w-5 text-start', 'orderable' => true, 'searchable' => false],
            ...
            if ($modules['company_id']) {
                // Hide company column if fixed company
                $columns = array_values(array_filter($columns, fn($c) => $c->name !== 'companies.company_name'));
            }

            View::share("columns", $columns);

            if ($request->ajax()) {
                // Build base query (NO default orderBy here — let DataTables handle it)
                $data = Branch::query();
                ...
                // JOIN so we can sort/search by company_name
                $data->select('branches.*', 'companies.company_name as company_name')
                ...
                $data = $data->orderBy('branches.created_at', 'desc');
                $returnData = Datatables::of($data)
                    ...
                    ->filter(function ($query) use ($request) {
                        if ($request->filled('filter_company')) {
                            $query->where('branches.company_id', $request->input('filter_company'));
                        }
                ...
                $returnData = $returnData->editColumn((new Branch())->getTable() . '.canteen_max_time', function ($row) use ($modules) {
                    return ($row?->canteen_max_time) ? $row?->canteen_max_time : '--';
                })
                    ->editColumn((new Branch())->getTable() . '.canteen_min_time', function ($row) use ($modules) {
                        return ($row?->canteen_min_time) ? $row?->canteen_min_time : '--';
                    })
    ...
    public function store(BranchRequest $request)
    {
        ...
        try {
            ...
            // Ensure optional fields are correctly set (already validated in BranchRequest)
            $validated['prefix'] = $request->input('prefix');
            $validated['branch_address'] = $request->input('branch_address');
            $validated['canteen_max_time'] = $request->input('canteen_max_time');
            $validated['canteen_min_time'] = $request->input('canteen_min_time');

            // Normalize employee_code_start to integer or null
            $employeeCodeStart = $request->input('employee_code_start');
            $validated['employee_code_start'] = $employeeCodeStart !== null && $employeeCodeStart !== ''
                ? (int) $employeeCodeStart
                : null;
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            if (!empty($modules['company_id'])) {
                $branchQuery->where('company_id', $modules['company_id']);
            }
            $edit = $branchQuery->findOrFail($id);
    ...
    public function update(BranchRequest $request, string $id)
    {
        ...
        try {
            ...
            $validated['prefix']=$request->prefix;
            $validated['branch_address']=$request->branch_address;
            $validated['canteen_min_time']=$request->canteen_min_time;
            $validated['canteen_max_time']=$request->canteen_max_time;
            // return $validated;
            $branchQuery = Branch::query();
            if (!empty($modules['company_id'])) {
                $branchQuery->where('company_id', $modules['company_id']);
            }
            $updateData = $branchQuery->findOrFail($id);
            if ($updateData) {
                unset($validated['id']);
    ...
    public function destroy(string $id, Request $request)
    {
        ...
        $dataDelete = $branchQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            $branchQuery = Branch::withTrashed();
            if (!empty($modules['company_id'])) {
                $branchQuery->where('company_id', $modules['company_id']);
            }
            $restore_data = $branchQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $country = $branchQuery->findOrFail($request?->id);
            if ($country) {
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            return view($modules['folder_path'] . '.print', compact('branch', 'company_id', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new BranchExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Branch-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
}
