...
use App\Exports\AssetsAllocationMasterExport;
use App\Helpers\Helper;
use App\Http\Controllers\Controller;
use App\Http\Requests\AssetsAllocationMasterRequest;
...
class AssetsAllocationMasterController extends Controller
{
    public $modules = [];
    public function __construct()
    {
        ...
        $this->modules = [
            'title' => 'Assets Allocation Master',
            'folder_path' => 'software.modules.master.assets-allocation-master',
            'route' => 'assets-allocation-master',
            'table_name' => (new AssetsAllocationMaster())->getTable(),
            'permisstion_prefix' => 'assets-allocation-master',
            'module_name' => 'Assets Allocation Master',
    ...
    public function index(Request $request)
    {
        ...
        try {
            // define columns
            $columns = [
                (object)['data' => 'DT_RowIndex', 'name' => 'DT_RowIndex', 'td_label' => 'Sr No.', 'className' => 'w-15', 'orderable' => false, 'searchable' => false],
                ...
                (object)['data' => 'name', 'name' => (new AssetsAllocationMaster())->getTable() . '.name', 'td_label' => 'Name', 'className' => '', 'orderable' => true, 'searchable' => false],
                (object)['data' => 'display_order', 'name' => (new AssetsAllocationMaster())->getTable() . '.display_order', 'td_label' => 'Display Order', 'className' => 'w-15', 'orderable' => true, 'searchable' => false],
                (object)['data' => 'status', 'name' => (new AssetsAllocationMaster())->getTable() . '.status', 'td_label' => 'Status', 'className' => 'w-15 text-start', 'orderable' => true, 'searchable' => false],
            ...
            if ($request->ajax()) {
                ...
                $data = AssetsAllocationMaster::select((new AssetsAllocationMaster())->getTable() . '.*', (new Company())->getTable() . '.company_name')
                    ->leftJoin('companies', (new Company())->getTable() . '.id', '=', (new AssetsAllocationMaster())->getTable() . '.company_id')
                ...
                $data = $data->orderBy((new AssetsAllocationMaster())->getTable() . '.id', 'DESC');

                $returnData = Datatables::of($data)
                    ->addIndexColumn()
                    ->filter(function ($query) use ($request) {
                        if ($request->has('filter_company') && $request->filter_company) {
                            $query->where((new AssetsAllocationMaster())->getTable() . '.company_id', $request->filter_company);
                        }
                        if ($request->has('status') && $request->status !== null && $request->status !== 'all') {
                            $query->where((new AssetsAllocationMaster())->getTable() . '.status', $request->status);
                        }
                        if ($request->has('search')) {
                            $query->where((new AssetsAllocationMaster())->getTable() . '.name', 'like', "%" . $request->search . "%");
                        }
                    });
     ...
     */
    public function store(AssetsAllocationMasterRequest $request)
    {
        $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
        ...
        try {
            ...
            // return $validated;
            AssetsAllocationMaster::create($validated);
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            $masterQuery = AssetsAllocationMaster::query();
            if (!empty($modules['company_id'])) {
                $masterQuery->where('company_id', $modules['company_id']);
            }
            $edit = $masterQuery->findOrFail($id);
     ...
     */
    public function update(AssetsAllocationMasterRequest $request, string $id)
    {
        $modules = $this->modules;
        $modules['authLoginUserDetail'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails : null;
        $modules['company_id'] = ($this->authenticateLoginUserDetails) ? $this->authenticateLoginUserDetails?->company_id : null;
        ...
        try {
            ...
            $updateData = $masterQuery->findOrFail($id);
            if ($updateData) {
                unset($validated['id']);
    ...
    public function destroy(string $id, Request $request)
    {
        ...
        $dataDelete = $masterQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            $masterQuery = AssetsAllocationMaster::withTrashed();
            if (!empty($modules['company_id'])) {
                $masterQuery->where('company_id', $modules['company_id']);
            }
            $restore_data = $masterQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $country = $masterQuery->findOrFail($request?->id);
            if ($country) {
    ...
    public function updateDisplayOrder(Request $request)
    {
        ...
        try {
            ...
            $validator = Validator::make($request?->all(), [
                ...
                'id' => ['required', Rule::exists((new AssetsAllocationMaster())->getTable(), 'id')],
                  'display_order' => [
                    'nullable',
                    Rule::unique((new AssetsAllocationMaster())->getTable(), 'display_order')
            ...
            if (AssetsAllocationMaster::where('company_id', $request?->cid)->where('display_order', '=', $newDisplayOrder)->count() && $newDisplayOrder) {
                // Step 1: Shift existing records that have display_order >= newDisplayOrder
                AssetsAllocationMaster::where('company_id', $request?->cid)->where('display_order', '>=', $newDisplayOrder)
                    ->increment('display_order');
            }

            $updatedMarketingStatus = AssetsAllocationMaster::where('id', $request->id)
                ->update([
    ...
    public function print(Request $request)
    {
        ...
        try {
            ...
            $query = AssetsAllocationMaster::select('*')
                ->where(function ($q1) use ($modules, $loginUserId) {
                    if (Auth::guard('employees')->check() || !empty($modules['company_id'])) {
                        // If company_id exists in $modules, use that; otherwise use employee's company_id
                        $companyId = $modules['company_id'] ?? Auth::guard('employees')->user()->company_id;
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(new AssetsAllocationMasterExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Assets Allocation Master-' . Helper::convert_date("", "Y-m-d H:i:s", "Ymd-His") . '.xlsx');
    }
}
