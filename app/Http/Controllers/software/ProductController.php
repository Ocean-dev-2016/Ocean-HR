...
class ProductController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modules = [
            ...
            'folder_path' => 'software.modules.master.product',
            'route' => 'products',
            'table_name' => (new Product())->getTable(),
            'permisstion_prefix' => 'Product Master',
    ...
    public function index(Request $request)
    {
        ...
        try {
            $columns = [
                ...
                (object)['data' => "operation", 'name' => 'operation', 'td_label' => 'Operation', 'className' =>  '', 'defaultContent' => '-'],
                (object)['data' => "process", 'name' => 'process', 'td_label' => 'Process', 'className' =>  '', 'defaultContent' => '-'],
                (object)['data' => "unit", 'name' => 'unit', 'td_label' => 'Unit', 'className' =>  '', 'defaultContent' => '-'],
                (object)['data' => "ot_text", 'name' => 'ot_text', 'td_label' => 'Ot text', 'className' =>  '', 'defaultContent' => '-'],
                (object)['data' => "rate", 'name' => 'rate', 'td_label' => 'Rate', 'className' =>  'text-end'],
                ...
                $data = Product::select('*')
                    ->where(function ($query) use ($modules, $loginUserId) {
                        if (Auth::guard('employees')->check() || !empty($modules['company_id'])) {
                            ...
                            $query->where((new Product())->getTable() . '.company_id', $companyId);

                            if ($modules['personal_data_permission'] && $modules['all_data_permission'] == false) {
                                $query->where((new Product())->getTable() . '.created_by', $loginUserId);
                            }
                        }
                    })
                    ->orderBy('id', 'DESC');
    ...
    public function edit(string $id)
    {
        ...
        try {
            ...
            if (!empty($modules['company_id'])) {
                $productQuery->where('company_id', $modules['company_id']);
            }
            $edit = $productQuery->findOrFail($id);
    ...
    public function update(ProductRequest $request, string $id)
    {
        ...
        try {
            ...
            $updateData = $productQuery->findOrFail($id);
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        $dataDelete = $productQuery->findOrFail($id);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            $restore_data = $productQuery->findOrFail($id);
    ...
    public function status_update(Request $request)
    {
        ...
        try {
            ...
            $product = $productQuery->findOrFail($request->id);
            $product->status = $request->update_status;
            $product->save();
            
            if ($request->ajax()) {
                return $this->sendResponse($product, $modules['title'] . ' status updated successfully.');
            }
    ...
    public function print(Request $request)
    {
        ...
        try {
            $query = Product::select('*')
                ->where(function ($q) use ($modules, $loginUserId, $request) {
                    if (Auth::guard('employees')->check() || !empty($modules['company_id'])) {
                        $q->where('company_id', $modules['company_id']);
                    }
                    if ($request->has('operation') && $request->operation) {
                        $q->where('operation', $request->operation);
                    }
                    if ($request->has('status') && $request->status !== null && $request->status !== 'all') {
            ...
            $products = $query->get();
            return view($modules['folder_path'] . '.print', compact('products', 'modules'));
        } catch (\Exception $e) {
            return Redirect::route($modules['route'] . '.index')->withErrors($e->getMessage());
        }
    }
    ...
    {
        $modules = $this->modules;
        return Excel::download(new ProductExport($request->all(), $this->authenticateLoginUserDetails, $modules), 'Product-' . date("Ymd-His") . '.xlsx');
    }

    public function get_extra_data(Request $request)
    {
        return response()->json([
            'operations' => OperationsRateList::$statuses,
        ]);
    }
}
