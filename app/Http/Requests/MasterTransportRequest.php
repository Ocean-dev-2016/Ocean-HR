...
use App\Models\MasterTransportBy;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Http\Request;
use Illuminate\Validation\Rule;

class MasterTransportRequest extends FormRequest
{
    /**
    ...
    {
        $id = $request->route('master_transport_by') ?? 0;
        $rules = [
            'company_id' => [
            ...
            'name' => [
                'required',
                Rule::unique((new MasterTransportBy())->getTable())->where(function ($query) use ($request) {
